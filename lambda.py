...
def get_baseline(city_name, hour_of_day):
    try:
        response = BASELINES_TABLE.get_item(
            Key={
                'city_name': city_name,
                'hour_of_day': str(hour_of_day)# updated this in lambda and deployed it to match the baseline schema
            }
        )
...
def process_record(record_data):
    try:
        ...
        if not baseline:
            logger.info(f"No baseline for {city_name} hour {hour_of_day} — skipping")
            return

        z = calculate_z_score(current_temp, baseline.get('temp_mean'), baseline.get('temp_std'))
        ...
        logger.error(f"Missing field: {e} | record: {record_data}")
    except Exception as e:
        logger.error(f"Unexpected error: {e} | record: {record_data}")
...
def lambda_handler(event, context):
    ...
    logger.info(f"Batch complete: {processed_count}/{len(records)} processed")
